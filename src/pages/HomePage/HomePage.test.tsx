...
const getSearchInput = () =>
	screen.getByTestId("search-repositories").querySelector("input");
const getRows = () =>
	screen.getAllByRole("row", { name: "repositories-table" });
const getRowCells = (rowIndex: number) =>
	screen
		.getAllByRole("row", { name: "repositories-table" })
...
describe("HomePage integration tests", () => {
	...
	describe("after loading", () => {
		...
		it("should have react github repo link on first table row", () => {
			const cells = getRowCells(0);
			expect(cells[0].firstChild).toHaveAttribute(
				"href",
